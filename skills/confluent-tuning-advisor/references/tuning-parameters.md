...
> - **Defaults differ, and by version:** Java `linger.ms` is `0` before Kafka 4.0 and `5` from 4.0
>   on, versus `5` in librdkafka; `enable.idempotence` is `true` in Java but `false` in librdkafka;
>   `max.in.flight.requests.per.connection` is `5` in Java but `1000000` in librdkafka. The *baseline
>   you diff from* is client- and version-specific even when the recommended value is not.
