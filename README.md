...
| confluent-tuning-advisor | Recommend Kafka tuning for Confluent Cloud or Confluent Platform by workload priority (latency, throughput, availability, or durability), making the trade-off against the other axes explicit. Inspects topic, client, and broker configuration read-only via the Confluent CLI, an MCP server, or the Kafka Admin REST API, then presents current-to-recommended diffs with rationale, risks, and validation steps; advisory-only, the administrator applies the change. |
